...
class BPSDF():
    ...
    # def get_whole_body_sdf_batch_qe(self,x,pose,theta,qd,model,use_derivative = True, used_links = [0,1,2,3,4,5,6,7,8],return_index=False):
    ...
    #     x_robot_frame_batch = utils.transform_points(x.float(),torch.linalg.inv(fk_trans).float(),device=self.device)
    #     x_robot_frame_batch_scaled = x_robot_frame_batch - offset.unsqueeze(1)
    #     x_robot_frame_batch_scaled = x_robot_frame_batch_scaled/scale.unsqueeze(-1).unsqueeze(-1)
    #     x_bounded = torch.where(x_robot_frame_batch_scaled>1.0-1e-2,1.0-1e-2,x_robot_frame_batch_scaled)
    #     x_bounded = torch.where(x_bounded<-1.0+1e-2,-1.0+1e-2,x_bounded)
    #     res_x_theta = x_robot_frame_batch_scaled - x_bounded

    #     if not use_derivative:
    #         phi,_ = self.build_basis_function_from_points(x_bounded.reshape(B*K*N,3), use_derivative=False)
    #         phi = phi.reshape(B,K,N,-1).transpose(0,1).reshape(K,B*N,-1)
    #         weights_near = torch.cat([model[i]['weights'].unsqueeze(0) for i in used_links],dim=0).to(self.device)
    #         sdf_theta = torch.einsum('ijk,ik->ij',phi,weights_near).reshape(K,B,N).transpose(0,1).reshape(B*K,N)
    #         sdf_theta = sdf_theta + res_x_theta.norm(dim=-1)
    #         sdf_theta = sdf_theta.reshape(B,K,N)
    #         sdf_theta = sdf_theta*scale.reshape(B,K).unsqueeze(-1)
    #         sdf_theta_val, idx_theta = sdf_theta.min(dim=1)
    #         grad_theta_val = None
    #     else:
    #         phi,dphi = self.build_basis_function_from_points(x_bounded.reshape(B*K*N,3), use_derivative=True)
    #         phi_cat = torch.cat([phi.unsqueeze(-1),dphi],dim=-1)
    ...
    #         sdf_t = output[:,:,0]
    #         grad_t = output[:,:,1:]
    #         sdf_t = sdf_t + res_x_theta.norm(dim=-1)
    #         sdf_t = sdf_t.reshape(B,K,N)
    #         sdf_t = sdf_t*(scale.reshape(B,K).unsqueeze(-1))
    #         sdf_theta_val, idx_theta = sdf_t.min(dim=1)
    #         grad_t = res_x_theta + torch.nn.functional.normalize(grad_t,dim=-1)
    #         grad_t = torch.nn.functional.normalize(grad_t,dim=-1).float()
    #         fk_rotation = fk_trans[:,:3,:3]
    #         grad_t_base = torch.einsum('ijk,ikl->ijl',fk_rotation,grad_t.transpose(1,2)).transpose(1,2).reshape(B,K,N,3)
    #         idx_grad = idx_theta.unsqueeze(1).unsqueeze(-1).expand(B,K,N,3)
    #         grad_theta_val = torch.gather(grad_t_base,1,idx_grad)[:,0,:,:]

    #     # ---------- 由 qd 计算 qe，并做前向 ----------
    #     qe_list = []
    #     for b in range(B):
    #         qe_b = BPSDF.compute_qe(theta[b].detach().cpu().numpy(), qd[b].detach().cpu().numpy())
    #         qe_list.append(qe_b)
    #     qe = torch.tensor(qe_list, device=self.device, dtype=torch.float32)

    #     trans_list_qe = self.robot.get_transformations_each_link(pose,qe)
    #     fk_trans_qe = torch.cat([t.unsqueeze(1) for t in trans_list_qe],dim=1)[:,used_links,:,:].reshape(-1,4,4)
    #     x_robot_frame_batch_qe = utils.transform_points(x.float(),torch.linalg.inv(fk_trans_qe).float(),device=self.device)
    #     x_robot_frame_batch_scaled_qe = x_robot_frame_batch_qe - offset.unsqueeze(1)
    #     x_robot_frame_batch_scaled_qe = x_robot_frame_batch_scaled_qe/scale.unsqueeze(-1).unsqueeze(-1)
    #     x_bounded_qe = torch.where(x_robot_frame_batch_scaled_qe>1.0-1e-2,1.0-1e-2,x_robot_frame_batch_scaled_qe)
    #     x_bounded_qe = torch.where(x_bounded_qe<-1.0+1e-2,-1.0+1e-2,x_bounded_qe)
    #     res_x_qe = x_robot_frame_batch_scaled_qe - x_bounded_qe

    #     if not use_derivative:
    #         phi_qe,_ = self.build_basis_function_from_points(x_bounded_qe.reshape(B*K*N,3), use_derivative=False)
    #         phi_qe = phi_qe.reshape(B,K,N,-1).transpose(0,1).reshape(K,B*N,-1)
    #         weights_near = torch.cat([model[i]['weights'].unsqueeze(0) for i in used_links],dim=0).to(self.device)
    #         sdf_qe = torch.einsum('ijk,ik->ij',phi_qe,weights_near).reshape(K,B,N).transpose(0,1).reshape(B*K,N)
    #         sdf_qe = sdf_qe + res_x_qe.norm(dim=-1)
    #         sdf_qe = sdf_qe.reshape(B,K,N)
    #         sdf_qe = sdf_qe*scale.reshape(B,K).unsqueeze(-1)
    #         sdf_qe_val, idx_qe = sdf_qe.min(dim=1)
    #         grad_qe_val = None
    #     else:
    #         phi_qe,dphi_qe = self.build_basis_function_from_points(x_bounded_qe.reshape(B*K*N,3), use_derivative=True)
    #         phi_qe_cat = torch.cat([phi_qe.unsqueeze(-1),dphi_qe],dim=-1)
    #         phi_qe_cat = phi_qe_cat.reshape(B,K,N,-1,4).transpose(0,1).reshape(K,B*N,-1,4)
    #         weights_near = torch.cat([model[i]['weights'].unsqueeze(0) for i in used_links],dim=0).to(self.device)
    #         output_qe = torch.einsum('ijkl,ik->ijl',phi_qe_cat,weights_near).reshape(K,B,N,4).transpose(0,1).reshape(B*K,N,4)
    #         sdf_q = output_qe[:,:,0]
    #         grad_q = output_qe[:,:,1:]
    #         sdf_q = sdf_q + res_x_qe.norm(dim=-1)
    #         sdf_q = sdf_q.reshape(B,K,N)
    #         sdf_q = sdf_q*(scale.reshape(B,K).unsqueeze(-1))
    #         sdf_qe_val, idx_qe = sdf_q.min(dim=1)
    #         grad_q = res_x_qe + torch.nn.functional.normalize(grad_q,dim=-1)
    #         grad_q = torch.nn.functional.normalize(grad_q,dim=-1).float()
    #         fk_rotation_qe = fk_trans_qe[:,:3,:3]
    #         grad_q_base = torch.einsum('ijk,ikl->ijl',fk_rotation_qe,grad_q.transpose(1,2)).transpose(1,2).reshape(B,K,N,3)
    #         idx_grad_qe = idx_qe.unsqueeze(1).unsqueeze(-1).expand(B,K,N,3)
    #         grad_qe_val = torch.gather(grad_q_base,1,idx_grad_qe)[:,0,:,:]

    #     # ---------- 选择逐点最小 ----------
    #     choose_theta = sdf_theta_val <= sdf_qe_val
    #     sdf_min = torch.where(choose_theta, sdf_theta_val, sdf_qe_val)

    #     grad_min = None
    #     if use_derivative and (grad_theta_val is not None) and (grad_qe_val is not None):
    #         grad_min = torch.where(choose_theta.unsqueeze(-1), grad_theta_val, grad_qe_val)

    #     if return_index:
    #         idx_min = torch.where(choose_theta, idx_theta, idx_qe)
    #         return sdf_min, grad_min, idx_min
    #     else:
