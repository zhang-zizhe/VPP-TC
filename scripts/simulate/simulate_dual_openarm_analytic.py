...
def get_args():
    ...
    parser.add_argument("--ctrl-dt", type=float, default=0.001,
                        help="control period (s). 0.001=1ms (fixed), 0.02=orig 20ms")
    ...
    parser.add_argument("--barrier", type=str, default="analytic",
                        choices=["model", "analytic", "viability"],
                        help="analytic = geometric current-distance + 1/d CBF; "
                             "viability = min(d(q),d(qe)) braking-aware + 1/h "
                             "CBF (velocity-aware, faithful gradient); "
                             "model = GammaRegressor (legacy).")
    parser.add_argument("--bar-trigger", type=float, default=0.05,
                        help="analytic barrier engages when inter-arm distance "
                             "< this (m).  Smooth ramp engages EARLY (50mm).")
    parser.add_argument("--bar-floor", type=float, default=0.003,
                        help="floor (m) on d in the 1/d term to bound the gain.")
...
def analytic_barrier(robot, all_pos, arm_cols, left_links, right_links,
                     n_all_dof, trigger_d, h_floor):
    """Geometric inter-arm-distance barrier with a 1/d reciprocal CBF.

    NO learned model.  Returns (d_report, grad28 or None).  grad28 is the
    gradient of the reciprocal barrier B = -1/h w.r.t. [q, qd]:
      gq = (1/h^2) * dd/dq     (qd-part = 0, distance depends on q only)
    dd/dq is the exact contact-Jacobian gradient of the inter-arm distance.
    grad is None (avoidance off) when d >= trigger_d.
    """
    zeros = [0.0] * n_all_dof
    ...
    n_AtoB = sep / ns if ns > 1e-9 else -nB   # direction of increasing d

    sA = p.getLinkState(robot, lA, computeForwardKinematics=True)
    sB = p.getLinkState(robot, lB, computeForwardKinematics=True)
    invA = p.invertTransform(sA[0], sA[1])
    invB = p.invertTransform(sB[0], sB[1])
    localA = p.multiplyTransforms(invA[0], invA[1], pA.tolist(), [0, 0, 0, 1])[0]
    localB = p.multiplyTransforms(invB[0], invB[1], pB.tolist(), [0, 0, 0, 1])[0]
    JA = np.array(p.calculateJacobian(robot, lA, list(localA),
                                      all_pos, zeros, zeros)[0])
    JB = np.array(p.calculateJacobian(robot, lB, list(localB),
                                      all_pos, zeros, zeros)[0])
    dd_dq_all = n_AtoB @ (JB - JA)            # over all movable joints
    dd_dq = dd_dq_all[arm_cols]               # 14 arm joints
    h = max(d, h_floor)
    gq = (1.0 / (h * h)) * dd_dq              # grad of B = -1/h
    grad28 = np.concatenate([gq, np.zeros(14)])
    return d, grad28


def _dist_grad(robot, all_pos_for_jac, arm_cols, left_links, right_links,
               n_all_dof):
    """Min inter-arm distance + exact contact-Jacobian gradient dd/dq (14).
    Robot must already be at the pose (live state) for getClosestPoints;
    all_pos_for_jac must hold that same pose for calculateJacobian."""
    zeros = [0.0] * n_all_dof
    best_d, best = 1e9, None
    for l1 in left_links:
        for l2 in right_links:
            for pt in p.getClosestPoints(robot, robot, 2.0, l1, l2):
                if pt[8] < best_d:
                    best_d, best = pt[8], pt
    if best is None:
        return 1e9, np.zeros(14)
    d = best[8]
    lA, lB = best[3], best[4]
    pA, pB, nB = np.array(best[5]), np.array(best[6]), np.array(best[7])
    sep = pB - pA
    ns = np.linalg.norm(sep)
    n_AtoB = sep / ns if ns > 1e-9 else -nB
    sA = p.getLinkState(robot, lA, computeForwardKinematics=True)
    sB = p.getLinkState(robot, lB, computeForwardKinematics=True)
    invA = p.invertTransform(sA[0], sA[1])
    invB = p.invertTransform(sB[0], sB[1])
    ...
    localB = p.multiplyTransforms(invB[0], invB[1], pB.tolist(), [0, 0, 0, 1])[0]
    JA = np.array(p.calculateJacobian(robot, lA, list(localA),
                                      all_pos_for_jac, zeros, zeros)[0])
    JB = np.array(p.calculateJacobian(robot, lB, list(localB),
                                      all_pos_for_jac, zeros, zeros)[0])
    return d, (n_AtoB @ (JB - JA))[arm_cols]


def viability_barrier(robot, all_pos, arm_cols, movable_joints, left_links,
                      right_links, n_all_dof, q_np, qd_np, qe, acc_max,
                      trigger_d, h_floor):
    """Viability barrier on h = min(d(q), d(qe)), qe = braking stop pose.
    Velocity-aware (qe depends on qd) AND faithful gradient (contact Jacobian),
    with a 1/h^2 reciprocal CBF for near-boundary authority.
    Returns (h_report, grad28 or None).

    Chain rule on the qe term:  qe_j = q_j + 0.5*qd_j*|qd_j|/a_j
      d(qe)/dq_j  = (dd/dqe)_j               (dqe/dq = 1)
      d(qe)/dqd_j = (dd/dqe)_j * |qd_j|/a_j  (dqe/dqd = |qd|/a)
    """
    # --- current pose (robot is live at q) ---
    d_q, dd_q = _dist_grad(robot, all_pos, arm_cols, left_links, right_links,
                           n_all_dof)
    # --- braking pose qe: excursion (MUST restore pos AND vel) ---
    all_pos_qe = list(all_pos)
    for k, c in enumerate(arm_cols):
        all_pos_qe[c] = float(qe[k])
    for jid, val in zip(movable_joints, qe):
        p.resetJointState(robot, jid, float(val), targetVelocity=0.0)
    d_qe, dd_qe = _dist_grad(robot, all_pos_qe, arm_cols, left_links,
                             right_links, n_all_dof)
    for k, jid in enumerate(movable_joints):
        p.resetJointState(robot, jid, float(q_np[k]),
                          targetVelocity=float(qd_np[k]))

    if d_q <= d_qe:
        h_raw, gq, gqd = d_q, dd_q, np.zeros(14)
    else:
        h_raw = d_qe
        gq = dd_qe
        gqd = dd_qe * (np.abs(qd_np) / acc_max)
    if h_raw >= trigger_d:
        return h_raw, None
    h = max(h_raw, h_floor)
    s = 1.0 / (h * h)
    return h_raw, np.concatenate([s * gq, s * gqd])
...
def main():
    ...
    if args.barrier == "model":
        ...
        print(f"  [barrier] {args.barrier.upper()}  "
              f"trigger={args.bar_trigger*1000:.0f}mm  floor={args.bar_floor*1000:.0f}mm")
    ...
    CTRL_DT = args.ctrl_dt   # configurable; 0.001=1ms fixed, 0.02=orig 20ms
    DECIM = max(1, round(CTRL_DT / args.stepsize))   # viability box dt stays 0.02
    print(f"  [ctrl] decimation={DECIM} physics steps / control solve")
    ...
    for step_i in range(num_steps):
        ...
        # ---- Barrier: analytic / viability (geometric, faithful) OR model ----
        if args.barrier == "analytic":
            Gamma, grad_gamma = analytic_barrier(
                robot, all_pos, arm_indices_in_all,
                left_body_links, right_body_links, n_all_dof,
                args.bar_trigger, args.bar_floor)
        elif args.barrier == "viability":
            Gamma, grad_gamma = viability_barrier(
                robot, all_pos, arm_indices_in_all, movable_joints,
                left_body_links, right_body_links, n_all_dof,
                q_np, qd_np, qe, acc_max, args.bar_trigger, args.bar_floor)
        else:
            Gamma, grad_gamma = compute_gamma_and_grad(
                gamma_model, q, qd, args.gamma_threshold, device)

        if step_i % 100 == 0:
            print(f"[sim] t={sim_t:6.3f}s  d={Gamma*1000:+7.2f}mm  "
        ...
        for _ in range(DECIM):   # zero-order hold control torque over physics steps
            p.setJointMotorControlArray(
