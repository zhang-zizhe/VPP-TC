...
if __name__ == "__main__":
    ...
    lambda1, lambda2, lambda3 = 5, 10, 10
    alpha = 1e-3
    ...
    target_pos = np.array([1.8, 0.0, 0.6])
    dt = 0.02  
    rospy.sleep(1.0)
    t0 = time.time()
    while not rospy.is_shutdown():
        ...
        gamma_val, grad_gamma = functions_collision.compute_gamma_and_grad2(
            np.array(q, dtype=np.float32), np.array(q_dot, dtype=np.float32), threshold=3
        )
