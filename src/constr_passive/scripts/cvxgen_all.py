...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        # target_frame = rospy.get_param("~target_frame", "panda_link0")sss
        ...
        # obs_pub1.publish(make_sphere_marker(obs_pos, scale=0.08, color=(1.0, 0.0, 0.0)))
        ...
        gamma_val, grad_gamma = functions_collision.compute_gamma_and_grad(
            np.array(q, dtype=np.float32), np.array(q_dot, dtype=np.float32), threshold=14.4
        )
        ...
        fx = -1.5* (np.array(end_pos) - target_pos)
        ...
        if not soft:
            if grad_gamma is not None:
                ...
                # eps     = ((1.25-(gamma_val*0.1))**0.5)*0.5
                e_in = [eps]
                try:
                    target_torque = CVXsolver_sca(J_in, fc_in, M_in, b_in, qdd_lb_in, qdd_ub_in, list(g_eff), e_in, c_in)
