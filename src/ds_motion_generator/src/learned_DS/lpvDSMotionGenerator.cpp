...
lpvDSMotionGenerator::lpvDSMotionGenerator(ros::NodeHandle &n,
                                     ...
                                     std::vector<double> attractor_all,
...
bool lpvDSMotionGenerator::Init() {
    ...
    // FOR MITSUBISHI PICK-SCAN-CHECK-PLACE TASK (SHOULD BE READ/DEFINED in yaml OR dynamically!)
 ...
 //    q_att_(3) = 0; // msg_real_pose_.orientation.z;
    ...
    // FOR FRANKA COOKING TASK (SHOULD BE READ/DEFINED in yaml OR dynamically!)
	// q_att_(0) = 0.5464146463082739; //msg_real_pose_.orientation.w;
 //    q_att_(1) = -0.8061014645546491; //msg_real_pose_.orientation.x;
 //    q_att_(2) = -0.0847523085928732; // msg_real_pose_.orientation.y;
 //    q_att_(3) = -0.21081503635574203; // msg_real_pose_.orientation.z;


    // FOR FRANKA INSPECTION TASK (SHOULD BE READ/DEFINED in yaml OR dynamically!)
	// q_att_(0) = 0.5464146463082739; //msg_real_pose_.orientation.w;
 //    q_att_(1) = -0.8061014645546491; //msg_real_pose_.orientation.x;
 //    q_att_(2) = -0.0847523085928732; // msg_real_pose_.orientation.y;
 //    q_att_(3) = -0.21081503635574203; // msg_real_pose_.orientation.z;
...
bool lpvDSMotionGenerator::InitializeDS() {
    ...
    if (attractor_.size() != M_) {
		ROS_ERROR_STREAM("InitializeDS: Please provide 3 elements for the attractor. It has " << attractor_.size() << " elements.");
		return false;
	}

	if (attractor_all_.size() < M_) {
		ROS_ERROR_STREAM("InitializeDS: Please provide more than 3 elements for the attractor. It has " << attractor_all_.size() << " elements.");
		return false;
	}
	...
	for (int i = 0; i < attractor_.size(); i++) {
		target_pose_(i) = attractor_all_[i];
...
void lpvDSMotionGenerator::ComputeDesiredVelocity() {
	...
	// if (ds_id_ == 2 && start_ds_){
	// 	desired_velocity_(2) = 0.25;
	// 	ROS_WARN_THROTTLE(1, "UP MOTION FIRST.");
	// 	if (real_pose_(2) > 0.245)
	// 		start_ds_ = false;
	// }else{
		...
		/* NOMINAL DS: This works for the above scenarios */
		desired_velocity_ = LPV_DS_->compute_f(real_pose_ - (target_pose_ - target_offset_ - learned_att_), learned_att_);

		// Boundaries to avoid collisions with pick station due to end-effector geometry!
		if (ds_id_ == 2 && real_pose_(1) < -0.25){
			desired_velocity_(1) = 0.155;
			if (desired_velocity_(2) < 0)
				desired_velocity_(2) = 0.0025;
		}

		if (M_== 3)
			ROS_WARN_STREAM_THROTTLE(1, "Desired Velocities before limits:" << desired_velocity_(0) << " " << desired_velocity_(1) << " " << desired_velocity_(2));
		else
		...
		ROS_WARN_STREAM_THROTTLE(1, "Desired speed:" << desired_velocity_.Norm());
		...
		// // Increase if too low near attractor!
		// A_o_      = MatrixXd::Zero(3,3);
		// A_o_(0,0) = -1.5;
		// A_o_(1,1) = -1.5;
		// A_o_(2,2) = -1.5;
		// Vector3d linear_error;
		// linear_error(0) = real_pose_[0] - target_pose_[0];
		// linear_error(1) = real_pose_[1] - target_pose_[1];
		// linear_error(2) = real_pose_[2] - target_pose_[2];
		// omega_ = A_o_*linear_error;
		// ROS_WARN_STREAM_THROTTLE(1, "Linear DS speed:" <<omega_.norm() );  
		// double r = exp(-100* pos_error_);  	
		// ROS_WARN_STREAM_THROTTLE(1, "rbf:" << r); 
	...
	msg_desired_velocity_filtered_.linear.x  = desired_velocity_(0);
	msg_desired_velocity_filtered_.linear.y  = desired_velocity_(1);

	if (M_== 3){
		// msg_desired_velocity_filtered_.linear.z  = desired_velocity_filtered_(2);
		msg_desired_velocity_filtered_.linear.z  = desired_velocity_(2);
	...
	// Should apply a filter here too, either CDDynamics or an exponential!!!
...
void lpvDSMotionGenerator::PublishFuturePath() {
	...
	// This works for KUKA + Mitsubishi
	// msg.header.frame_id = "base_link";

	// This works for Franka ONLY
    ...
    if (bPublish_DS_path_){
        ...
        // This works for KUKA + Mitsubishi
        // msg_DesiredPath_.header.frame_id = "base_link";
        // This works for Franka ONLY
        msg_DesiredPath_.header.frame_id = "panda_link0";
        ...
        //    simulated_vel = LPV_DS_->compute_f(simulated_pose, target_pose_- target_offset_);
			...
			// This works for KUKA + Mitsubishi
            // msg_DesiredPath_.poses[frame].header.frame_id = "base_link";
			// This works for Franka ONLY
        	msg_DesiredPath_.poses[frame].header.frame_id = "panda_link0";
