...
# 三个 CSV 文件的路径
file_paths = [
    "../output/1757477084.7563334.csv",
    "../output/1757475026.7022321.csv"
]
labels = ["w/o SCA", "w/ SCA"]
color_map = {"w/o SCA": "tab:orange", "w/ SCA": "tab:green"}
...
ax_gamma.axhline(y=2.5, color="red", linestyle="--", linewidth=4)
ax_gamma.text(
    x=ax_gamma.get_xlim()[1] - 0.9 * (ax_gamma.get_xlim()[1] - ax_gamma.get_xlim()[0]),
    y=2.5 + 0.3,
    s=r"threshold",
    color="red",
    fontsize=35  # 这里单独指定“阈值”注释的字体大小
)
ax_gamma.set_ylim(-1, 20)
ax_gamma.set_xlim(0, 6)
ax_gamma.set_xlabel(r"Time [s]")
ax_gamma.set_ylabel(r"$\Gamma (q, \dot q)$")
...
ax_dist.text(
    x=ax_dist.get_xlim()[1] - 0.9 * (ax_dist.get_xlim()[1] - ax_dist.get_xlim()[0]),
    y=0.0012,
    s=r"collision",
    color="red",
    fontsize=35  # 这里单独指定“阈值”注释的字体大小
)
ax_dist.set_ylim(-0.01, 0.08)
...
# 保存图像（DPI 可根据需要调整）
plt.savefig("../output/plot_sca.png", dpi=1300, bbox_inches="tight")
# plt.show()
